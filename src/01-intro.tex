...
In this work, an edge computing system with multiple APs and edge servers is considered, which resides in the Metropolitan Area Network (MAN).
...
In \cite{TWC18-LyuX}, the above approach is applied to the scenario that mobile devices offload jobs to each others via the D2D link.
...
%NOTE: Contributions
In this work, the signaling latency among APs and edge servers and job uploading latency from APs to edge servers are assumed to be random, and hence each AP can only observe part of the system state with random latency.
A novel low-complexity distributed solution framework for the dispatcher design is proposed at the APs, where each AP collects the information only from the APs and edge servers in a close proximity and make dispatching decision with random signaling latency.
The distributed dispatching policy directly obtained with one-step alternative policy iteration via the approximated value function.
Thus, the complicated POMDP solution or value iteration is avoided.
To our best knowledge, this is the first work to address the cooperative distributed multi-agent optimization problem under MDP framework.
