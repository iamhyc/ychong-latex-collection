...
\label{sec:model}
The system model of edge computing networks is elaborated in this section, where the randomness of job arrivals, uploading latency and computation time is considered.
Moreover, the distributed job dispatching model with partially observable system state information for each AP is elaborated.
\comments{At the end of this section, the problem is firstly formulated as a global problem which suffers from lack of global information and random \brlatency.}

\begin{figure*}[htp!]
    \centering
    \includegraphics[width=0.85\textwidth]{system-model.pdf}
...
In this work, an edge computing system with $K$ Access Points (APs) and $M$ edge servers is considered, which are connected in a network as illustrated in Fig.\ref{fig:system}.
...
Let $\esSet_{k}$ denote the \emph{candidate server set} of the $k$-th AP, and $\apSet_{m}$ as the \emph{potential AP set} of the $m$-th edge server ($\forall k\in\apSet, m\in\esSet$).
...
The time axis of the system is organized by time slots.
...
Furthermore, the \emph{unrelated machines assumption} is adopted as in \cite{tan-online}, and assume that the computation time of different job types on different edge servers follows independent memoryless geometric distribution 
...
In order to facilitate distributed dispatching for the APs, APs broadcast every $t_B$ time slots which is denoted as a \emph{broadcast interval}.
As illustrated in Fig.\ref{fig:brd-timeline}, at the beginning of each broadcast interval, the local state information (LSI) of APs and edge servers are broadcast, and each AP updates its strategy of job dispatching when observing the broadcast LSIs from some APs and edge servers.
...
\begin{definition}[Local State Information of APs]
    ...
    where $\vec{R}^{(k)}_{m,j}(t,0) \define \Big(
        R^{(k)}_{m,j}(0,t,0), \dots, R^{(k)}_{m,j}(\Xi,t,0)
    \Big)$, and $\mathcal{A}_{k}(t) \define \{\omega_{k,j}(t) | \forall j\in\jSpace\}$ is referred as the dispatching actions of the $k$-th AP at the start of the $t$-th broadcast interval.
\end{definition}
...
The dispatching design based on the observation of \emph{observed state information} at each AP is given as follows.
\begin{definition}[Observed State Information]
    ...
    \label{def:OSI}
\end{definition}
Thus, the \emph{conflict AP sets} of each AP is defined as follows.
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.85\textwidth]{brd-timeline.pdf}
...
It is assumed that $\mathcal{D}_{k}(t)$ follows identical and independent distribution in different broadcast interval, where $\mathcal{D}_{k}(t)$ denotes the \brlatency~of the $k$-th AP at the $t$-th broadcast interval.
