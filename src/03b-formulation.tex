...
%----------------------------------------------------------------------------------------%
In this section, the distributed job dispatching problem is formulated under standard MDP framework.
...
Hence, the cost function per broadcast interval is defined as follows.
...
Since the job dispatching in one broadcast interval will affect the GSI of the following broadcast intervals, the joint minimization of the discounted sum of the costs of all the broadcast intervals is taken as the objective, which is given as follows.
\begin{align}
...
This is because each AP (say the $k$-th AP) only has the knowledge of its own OSI $\Stat_{k}(t)$ and local \brlatency~$\mathcal{D}_{k}(t)$, and thus problem P1 is actually a POMDP.
...
In this work, a novel low-complexity solution framework is proposed with alternative actions update, where distributed job dispatching via the Bellman's equations becomes feasible even with OSI and local signaling latency.
%----------------------------------------------------------------------------------------%
