...
\begin{abstract}
    \comments{Edge computing system is the future, and it's deployed in the network with random network traffic.}    
    We consider the distributed job dispatching problem in an edge computing network residing in a Metropolitan Area Network (MAN), where the job arrivals, uploading latency and computation time are all random.
    ...
    \comments{The signaling model is designed that APs and edge servers periodically broadcast their local state information.}
    ...
    While conventional solution for POMDP is impractical due to huge complexity, we propose a novel low-complexity solution framework to address the issue of algorithm complexity.
